...
class MatchCenterPresenter(val model: MatchCenterModel): MatchCenterContract.MatchCenterPresenter{
    ...
    fun presentNextMatchInfo(match: SMMatch?){
        if(match!=null){
            ...
            this.view?.setNextMatchHomeTeam(match.localTeam.data.short_code, match.localTeam.data.logo_path)
            this.view?.setNextMatchAwayTeam(match.visitorTeam.data.short_code, match.visitorTeam.data.logo_path)
            ...
            if(startDT!= null) {
                ...
                if (timeDiff <= 0 || match.time.isLive) {
                    ...
                    this.view?.prepareMatchCenterTabs(true)

                } else {
                    val startDT = match.time.starting_at.startDateTime
                    this.view?.setNextMatchDate(Utilities.getLocaleFormattedDate(startDT), false)
                    this.view?.setNextMatchScore("-", "-")
                    this.view?.prepareMatchCenterTabs(false)
                }

                this.view?.setNextMatchTVGuide(match)
