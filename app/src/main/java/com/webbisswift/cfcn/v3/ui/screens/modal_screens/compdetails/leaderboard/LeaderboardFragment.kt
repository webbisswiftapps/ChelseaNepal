...
import kotlinx.android.synthetic.main.v3_fragment_league_leaderboard.*
...
class LeaderboardFragment : BaseFragment(), LeaderboardContract.View{
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        val view = inflater.inflate(R.layout.v3_fragment_league_leaderboard, null, true)
        initView()
        return view
    }
    ...
    override fun setTopGS(items: List<SMLeagueCharts.GoalItem>) {
        ...
        for(item in items){
            val view = LayoutInflater.from(context).inflate(R.layout.v3_leaderboard_goals_item,goalsLBHolder, false)
    ...
    override fun setTopCS(items: List<SMLeagueCharts.CardItem>) {
        ...
        for(item in items){
            val view = LayoutInflater.from(context).inflate(R.layout.v3_leaderboard_goals_item,cardsLBHolder, false)
            val playerName = view.findViewById<TextView>(R.id.playerName)
            val playerClub = view.findViewById<TextView>(R.id.playerTeam)
            val serialNo = view.findViewById<TextView>(R.id.serialNo)
            val goalCount = view.findViewById<TextView>(R.id.goalsCount)
    ...
    override fun setTopAS(items: List<SMLeagueCharts.AssistItem>) {
        ...
        for(item in items){
            val view = LayoutInflater.from(context).inflate(R.layout.v3_leaderboard_assists_item,assistsLBHolder, false)
            val playerName = view.findViewById<TextView>(R.id.playerName)
            val playerClub = view.findViewById<TextView>(R.id.playerTeam)
            val serialNo = view.findViewById<TextView>(R.id.serialNo)
