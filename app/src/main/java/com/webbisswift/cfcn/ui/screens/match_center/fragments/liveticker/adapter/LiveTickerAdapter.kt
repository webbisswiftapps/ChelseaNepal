    ...
    fun addComments(items: List<MatchComment>){
        ...
        if(items.size > 1) {
            for ((index, value) in items.withIndex()) {
                ...
                if(value.isgoal.contentEquals("1"))
                    type = TickerAdapterConstants.TYPE_GOAL
                else if(value.important.contentEquals("1"))
    ...
    override fun onCreateViewHolder(parent: ViewGroup?, viewType: Int): LiveTickerViewHolder {
        ...
        when(viewType){
            TickerAdapterConstants.TYPE_NORMAL ->{
                val view = LayoutInflater.from(context).inflate(R.layout.layout_ticker_normal_item, parent, false)
                return TickerCommentVH(view)
            }

            TickerAdapterConstants.TYPE_IMPORTANT ->{
                val view = LayoutInflater.from(context).inflate(R.layout.layout_ticker_important_item, parent, false)
                return TickerCommentVH(view)
            }

            TickerAdapterConstants.TYPE_GOAL ->{
                val view = LayoutInflater.from(context).inflate(R.layout.layout_ticker_goal_item, parent, false)
                return TickerCommentVH(view)
            }
